...
CartaTropa::CartaTropa(std::shared_ptr<DatosCartaTropa> datosBase)
	...
	DatosBase(datosBase), Municiones(0), Defensa(datosBase->DefensaInicial) {
}
...
int CartaTropa::getDistanciaDelGolpe() const {
	return DatosBase->DistanciaDeGolpe;
}
...
void CartaTropa::RecibirDanio(int danio) {
	Defensa -= danio;
	if (Defensa < 0) Defensa = 0; // No puede tener defensa negativa
	std::cout << Nombre << " recibe " << danio << " de danio. Defensa restante: " << Defensa << ".\n";
}
