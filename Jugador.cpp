...
void Jugador::RobarCartas(int cantidad) {
	if (cantidad <= 0) {
		std::cout << "Cantidad invalida para robar cartas.\n";
		return;
	}
	...
	for (int i = 0; i < cantidad; ++i) {
		if (Mazo.empty()) {
			...
			break; // Sale del bucle si el mazo esta vacio
		}
...
void Jugador::RobarCarta() {
    if (!Mazo.empty()) {
        ...
        Mano.push_back(carta);
		Mazo.erase(Mazo.begin() + random);
		Energia--; // Reduce la energia al robar una carta

		std::cout << "Carta robada: " << carta->GetNombre() << "\n";
		std::cout << "Energia restante: " << Energia << "\n";
    }
    else {
		std::cout << "El mazo esta vacio, no puedes robar mas cartas.\n";
    }
...
void Jugador::JugarCarta(int indice) {
	if (indice < 0 || static_cast<std::size_t>(indice) >= Mano.size()) {
...
void Jugador::RecibirDanio(int cantidad) {
	...
	if (Vida < 0) {
		...
		std::cout << Nombre << " recibio " << cantidad << " puntos de da�o. \n Vida restante: " << Vida << "\n";
	}
}
...
void Jugador::RegenerarEnergia(int cantidad) {
    Energia += cantidad;
	std::cout << "\nEnergia reabastecida. Energia actual: " << Energia << "\n";
}
...
std::shared_ptr<Carta> Jugador::GetCarta(int indice) const {
    if (indice < 0 || static_cast<std::size_t>(indice) >= Mano.size()) return nullptr;
	std::shared_ptr<Carta> carta = Mano[indice];
    if (carta->GetCostoUso() <= Energia) {
        return carta;
    } else {
